...
def exportar_rango():
    ...
    if tipo == "csv":
        ...
        ruta = carpeta_empresa(nombre)
        archivo = ruta / f"Registro_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.csv"
        ...
        os.startfile(ruta)
        return f"CSV guardado en:<br>{archivo}"
    ...
    # ================= PDF =================
        ...
        carpeta = os.path.join(os.getcwd(), "informes")
        os.makedirs(carpeta, exist_ok=True)

        # 📄 Nombre del archivo
        filename = f"registro_{nombre}.pdf"
        ...
        elements.append(Paragraph("ACADEMIA DE IDIOMAS Y REFUERZO", styles["Heading2"]))
        ...
        # 🔁 Guardar buffer en fichero físico
...
def exportar_admin():
    ...
    if tipo == "csv":
        ...
        return send_file(
            output,
            as_attachment=True,
            download_name="registro_usuario.csv",
            mimetype="text/csv"
        )
...
# LOGO
        ...
        return send_file(
            buffer,
            as_attachment=True,
            download_name="registro_admin.pdf",
            mimetype="application/pdf"
        )
